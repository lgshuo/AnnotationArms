...
public class onSuccess {
    ...
    public onSuccess(Element element) throws IllegalArgumentException {
        if (element.getKind() != ElementKind.METHOD) {
            throw new IllegalArgumentException(
                    String.format("Only methods can be annotated with @%s",
                            com.lscs.lgs.annotationlib.annotation.onSuccess.class.getSimpleName()));
        }

        mExecutableElement = (ExecutableElement) element;

        urlType =mExecutableElement.getAnnotation(com.lscs.lgs.annotationlib.annotation.onSuccess.class).type()+ mExecutableElement.getAnnotation(com.lscs.lgs.annotationlib.annotation.onSuccess.class).url();
        ...
        if (parameters.size() > 0) {
            throw new IllegalArgumentException(
                    String.format("The method annotated with @%s must have no parameters",
                            com.annotation.onSuccess.class.getSimpleName()));
        }*/
    }
